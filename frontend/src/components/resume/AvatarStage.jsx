...
const AvatarStage = ({ type = "user", speaking = false, lottieAI, running = false }) => {
  ...
  if (type === "ai") {
    return (
      ...
      >
        {lottieAI ? (
          <Lottie play loop animationData={lottieAI} style={{ height: 160, width: 160 }} />
        ) : (
      ...
      <div className="relative w-80 h-80 rounded-full overflow-hidden border bg-black flex items-center justify-center">
        ...
        {running && (
          <button
            ...
            ${showVideo ? "bg-red-500 hover:bg-red-600" : "bg-black/60 hover:bg-black/80"}`}
          >
