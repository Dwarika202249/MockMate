      ...
      try {
        ...
        if (data.interviews && data.interviews.length > 0) {
          ...
          const transformedInterviews = data.interviews.map((interview) => ({
            ...
            role: interview.resume?.jobRole || "Unknown Role",
            score: interview.summary?.overallScore || 0,
          ...
          // Extract skill scores from answers' feedback
          if (data.interviews.length > 0) {
            const allScores = data.interviews
              .filter(
                (interview) =>
                  interview.summary && interview.summary.overallScore
              )
              .map((interview) => interview.summary.overallScore);

            // Analyze top keywords from feedback to build skill breakdown
            const skillMap = {};
            data.interviews.forEach((interview) => {
              if (interview.answers && Array.isArray(interview.answers)) {
                interview.answers.forEach((answer) => {
                  if (
                    answer.feedback &&
                    Array.isArray(answer.feedback.keywords)
                  ) {
            ...
            const calculatedSkills = Object.entries(skillMap)
              .map(([skill, totalScore]) => ({
                ...
                score: Math.min(
                  100,
                  Math.round(totalScore / data.interviews.length)
                ),
              }))
    ...
    <div className="p-4 sm:p-6 bg-gray-100 min-h-screen">
      ...
      {!loading && interviews.length > 0 && (
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-purple-600">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Average Score Over Last 10 Interviews
              </p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-4 mb-4 border border-purple-200">
            ...
            <p className="text-sm text-gray-600 mt-2">Current Average Score</p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-sm font-bold text-gray-800">React (85)</p>
              </div>
              <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                <p className="text-xs text-orange-700 font-semibold">
                  📈 Needs Focus
                </p>
                <p className="text-sm font-bold text-gray-800">Redux (60)</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 mt-1">Last 5 interviews</p>
        </div>
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-2 text-sm font-medium text-purple-600 hover:bg-purple-50 rounded-lg transition-colors"
                    >
                      View Details
        ...
        <div className="mt-4 text-center">
          <button
            ...
            className="text-purple-600 hover:text-purple-700 text-sm font-semibold flex items-center justify-center gap-1 mx-auto"
          >
            View All Interviews
            <MdKeyboardArrowRight size={18} />
          </button>
